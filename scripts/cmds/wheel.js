...
const parseAmount = (str) => {
  ...
  const suffixes = {
    ...
    tg: 1e93, utg: 1e96, dtg: 1e99, c: 1e303 // 𝑪𝒆𝒏𝒕𝒊𝒍𝒍𝒊𝒐𝒏 𝑩𝒂𝒃𝒚
  };
...
function formatMoney(num) {
  const suffixes = [
    ...
    { value: 1e48,  symbol: "𝑸𝑰𝑫" }, { value: 1e45,  symbol: "𝑸𝑨𝑫" }, { value: 1e42,  symbol: "𝑻𝑫" },
    ...
    { value: 1e21,  symbol: "𝑺𝑿" },  { value: 1e18,  symbol: "𝑸𝑰" },  { value: 1e15,  symbol: "𝑸𝑨" },
...
module.exports = {
  config: {
    ...
    category: "game",
    countDown: 10,
    shortDescription: "🎡 𝑼𝒍𝒕𝒓𝒂 𝒘𝒉𝒆𝒆𝒍 𝒘𝒊𝒕𝒉 𝒎𝒂𝒔𝒔𝒊𝒗𝒆 𝒔𝒖𝒇𝒇𝒊𝒙𝒆𝒔",
  ...
  onStart: async function ({ api, event, args, usersData }) {
    ...
    if (userData.data.lastWheelDate !== today) {
      userData.data.lastWheelDate = today;
      userData.data.wheelCount = 0;
    }

    if (userData.data.wheelCount >= dailyLimit) {
    ...
    if (!betAmount || betAmount <= 0) {
      return api.sendMessage(toBoldSerifItalic("❌ 𝑰𝒏𝒗𝒂𝒍𝒊𝒅 𝒃𝒆𝒕 𝒂𝒎𝒐𝒖𝒏𝒕! 𝑼𝒔𝒂𝒈𝒆: 𝒘𝒉𝒆𝒆𝒍 500"), threadID, messageID);
    }

    if (userData.money < betAmount) {
      return api.sendMessage(toBoldSerifItalic("💰 𝑰𝒏𝒔𝒖𝒇𝒇𝒊𝒄𝒊𝒆𝒏𝒕 𝒃𝒂𝒍𝒂𝒏𝒄𝒆! 𝒀𝒐𝒖 𝒉𝒂𝒗𝒆: ") + formatMoney(userData.money), threadID, messageID);
    }

    cooldowns.set(senderID, nowTime);
    userData.data.wheelCount += 1;
    await usersData.set(senderID, { data: userData.data });

    const loadingMsg = await api.sendMessage(toBoldSerifItalic(`🎰 𝑺𝒑𝒊𝒏𝒏𝒊𝒏𝒈... (${userData.data.wheelCount}/${dailyLimit})\n💵 𝑩𝒆𝒕: `) + formatMoney(betAmount), threadID, messageID);
...
${outcomeText}
💰 ${toBoldSerifItalic("𝑵𝒆𝒘 𝒃𝒂𝒍𝒂𝒏𝒄𝒆:")} ${formatMoney(newBalance)}
✨ ${toBoldSerifItalic("𝑹𝒆𝒎𝒂𝒊𝒏𝒊𝒏𝒈 𝒍𝒊𝒎𝒊𝒕:")} ${dailyLimit - userData.data.wheelCount}`.trim();
